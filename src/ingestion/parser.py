...
from src.models.indicator import (
    IndicatorType, BaseIndicator, IPv4Indicator, DomainIndicator,
    FileHashIndicator, URLIndicator, get_utc_now
)
    ...
    def parse_raw_indicator(raw_data: Dict[str, Any]) -> Optional[BaseIndicator]:
        ...
        try:
            ...
            if ind_type_str == IndicatorType.IPV4.value:
                ...
                return URLIndicator(
                    **base_kwargs,
                    domain_ref=raw_data.get("domain_ref")
                )
            else:
